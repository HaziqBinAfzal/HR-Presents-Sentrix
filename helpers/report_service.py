...
from helpers.report_intelligence import build_report_context
from helpers.report_sections import REPORT_INTELLIGENCE_CSS, render_intelligence_sections
...
def generate_html_report(project, analysis):
    """Generate a self-contained, branded, evidence-aware Sentrix HTML report."""
    ...
    intelligence_sections = render_intelligence_sections(context)
...
.footer{{display:flex;align-items:center;justify-content:center;gap:8px;color:var(--muted);font-size:12px;text-align:center;margin-top:26px}}.footer-logo{{width:48px;height:32px}}.page-break{{break-before:page;page-break-before:always}}
...
<section class="card page-break"><div class="section-kicker">Document navigation</div><h2>Contents</h2><ol class="toc"><li><a href="#executive-summary">Executive Summary</a></li><li><a href="#score-overview">Score Overview</a></li><li><a href="#project-profile">Project Profile</a></li><li><a href="#methodology">Methodology</a></li><li><a href="#risk-assessment">Risk Assessment</a></li><li><a href="#structured-security">Security Intelligence</a></li><li><a href="#structured-quality">Quality Intelligence</a></li><li><a href="#dependency-configuration">Dependencies & Configuration</a></li><li><a href="#standards">Standards Mapping</a></li><li><a href="#recommendations">Recommendations</a></li><li><a href="#quality-findings">Raw Quality Output</a></li><li><a href="#security-findings">Raw Security Output</a></li><li><a href="#complexity-findings">Complexity Findings</a></li><li><a href="#appendix">Appendix</a></li></ol></section>
<section class="card" id="executive-summary"><div class="section-kicker">Assessment</div><h2>Executive Summary</h2><div class="health-panel"><div><strong>Project Health</strong><p style="margin:6px 0 0;color:var(--muted)">{health_message}</p></div><span class="health {health_class}">{health_label}</span></div><h3>Analysis summary</h3><div class="summary">{_safe(getattr(analysis,'ai_summary',None),'No summary was generated for this analysis.')}</div><h3>Evidence-aware interpretation</h3><p>{_safe(context['executive_summary'])}</p></section>
...
<section class="card" id="quality-findings"><div class="section-kicker">Static analysis</div><h2>Raw Quality Findings</h2><pre class="output">{_preformatted(getattr(analysis,'pylint_output',None))}</pre></section>
<section class="card" id="security-findings"><div class="section-kicker">Security review</div><h2>Raw Security Findings</h2><pre class="output">{_preformatted(getattr(analysis,'bandit_output',None))}</pre></section>
