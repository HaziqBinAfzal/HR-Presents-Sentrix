...
from helpers.report_enrichment import (
    appendix_enrichment,
    complexity_enrichment,
    executive_enrichment,
    project_profile_enrichment,
    quality_enrichment,
    recommendations_enrichment,
    security_enrichment,
)
...
def generate_html_report(project, analysis):
    """Generate the existing Sentrix report with content-only technical enrichment."""
...
<style>
:root {{ --primary:#1677ff; --primary-dark:#0f4fb8; --primary-soft:#eaf3ff; --dark:#0f172a; --muted:#64748b; --border:#dbe3ee; --surface:#ffffff; --background:#f4f7fb; --success:#15803d; --warning:#b45309; --danger:#dc2626; }}
* {{ box-sizing:border-box; }} html {{ scroll-behavior:smooth; }}
body {{ margin:0; font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif; background:var(--background); color:var(--dark); line-height:1.6; }}
a {{ color:var(--primary-dark); text-decoration:none; }}
.report {{ max-width:1120px; margin:32px auto; padding:0 24px 40px; }}
.cover {{ min-height:690px; display:flex; flex-direction:column; justify-content:space-between; background:linear-gradient(145deg,#08111f 0%,#15335d 62%,#1677ff 100%); color:white; border-radius:22px; padding:46px; box-shadow:0 20px 56px rgba(15,23,42,.22); }}
.brand {{ font-size:30px; font-weight:800; letter-spacing:-.02em; }} .brand small {{ display:block; font-size:10px; letter-spacing:.18em; opacity:.76; margin-top:3px; }}
.cover h1 {{ margin:0 0 12px; font-size:48px; line-height:1.08; }} .cover-subtitle {{ font-size:20px; opacity:.88; max-width:680px; }}
...
.actions {{ display:flex; justify-content:flex-end; gap:10px; margin:18px 0; }} .button {{ border:0; border-radius:10px; padding:11px 18px; font-weight:700; cursor:pointer; }} .button-primary {{ background:var(--primary); color:white; }} .button-secondary {{ background:white; color:var(--dark); border:1px solid var(--border); }}
.grid {{ display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:16px; margin:22px 0; }} .metric,.card {{ background:var(--surface); border:1px solid var(--border); border-radius:16px; box-shadow:0 8px 24px rgba(15,23,42,.05); }} .metric {{ padding:20px; }} .metric-label {{ color:var(--muted); font-size:13px; margin-bottom:8px; }} .metric-value {{ font-size:27px; font-weight:800; }}
.card {{ padding:26px; margin-top:18px; }} .card h2 {{ margin:0 0 18px; font-size:22px; }} .card h3 {{ margin:22px 0 10px; font-size:17px; }}
.section-kicker {{ color:var(--primary-dark); font-size:12px; font-weight:800; letter-spacing:.12em; text-transform:uppercase; }}
.table {{ width:100%; border-collapse:collapse; }} .table th,.table td {{ padding:12px 10px; border-bottom:1px solid var(--border); text-align:left; vertical-align:top; }} .table th {{ width:220px; color:var(--muted); font-size:13px; font-weight:600; }} .table tr:last-child th,.table tr:last-child td {{ border-bottom:0; }}
...
.summary {{ background:var(--primary-soft); border-left:4px solid var(--primary); padding:18px; border-radius:10px; }} .health-panel {{ display:flex; justify-content:space-between; gap:20px; align-items:center; padding:18px; border-radius:12px; background:#f8fafc; border:1px solid var(--border); }}
.toc {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px 28px; padding:0; list-style:none; }} .toc li {{ border-bottom:1px dashed var(--border); padding:8px 0; }} .recommendations {{ margin:0; padding-left:22px; }} .recommendations li {{ padding:7px 0 7px 5px; }}
...
.appendix-note {{ color:var(--muted); font-size:13px; }} .footer {{ color:var(--muted); font-size:12px; text-align:center; margin-top:26px; }} .page-break {{ break-before:page; page-break-before:always; }} .avoid-break {{ break-inside:avoid; page-break-inside:avoid; }}
@media (max-width:820px) {{ .grid {{ grid-template-columns:repeat(2,minmax(0,1fr)); }} .cover {{ min-height:auto; }} .cover-meta,.toc {{ grid-template-columns:1fr; }} .health-panel {{ align-items:flex-start; flex-direction:column; }} }}
@media (max-width:520px) {{ .report {{ padding:0 12px 24px; margin-top:12px; }} .cover {{ padding:28px; }} .cover h1 {{ font-size:34px; }} .grid {{ grid-template-columns:1fr; }} .table th,.table td {{ display:block; width:100%; }} .table th {{ padding-bottom:3px; border-bottom:0; }} }}
@media print {{ @page {{ size:A4; margin:14mm; }} body {{ background:white; print-color-adjust:exact; -webkit-print-color-adjust:exact; }} .report {{ max-width:none; margin:0; padding:0; }} .cover {{ min-height:252mm; border-radius:0; box-shadow:none; }} .actions {{ display:none; }} .metric,.card {{ box-shadow:none; }} .output {{ max-height:none; overflow:visible; color:#111827; background:#f8fafc; border:1px solid var(--border); }} a {{ color:inherit; }} }}
</style>
</head>
<body>
<main class="report">
<section class="cover"><div class="brand">🛡 Sentrix<small>PRESENTED BY HR-PRESENTS</small></div><div><div class="section-kicker" style="color:#b9d6ff;">Software assurance report</div><h1>Project Analysis Report</h1><p class="cover-subtitle">Quality, security, maintainability, and structural findings for {_safe(project.project_name)}.</p><div class="cover-meta"><div><small>Project</small><strong>{_safe(project.project_name)}</strong></div><div><small>Report ID</small><strong>SENTRIX-{analysis.id}</strong></div><div><small>Language</small><strong>{_safe(analysis.language,'Python')}</strong></div><div><small>Generated</small><strong>{created_display}</strong></div></div></div><div class="confidential">Confidential analysis output · Generated for the authenticated Sentrix workspace owner</div></section>
...
<section class="card" id="project-profile"><div class="section-kicker">Scope</div><h2>Project Profile</h2><table class="table"><tr><th>Project</th><td>{_safe(project.project_name)}</td></tr><tr><th>Original File</th><td>{_safe(project.original_filename)}</td></tr><tr><th>Language</th><td>{_safe(analysis.language)}</td></tr><tr><th>Status</th><td>{_safe(analysis.status)}</td></tr><tr><th>Analysis Date</th><td>{created_display}</td></tr><tr><th>Duration</th><td>{_safe(getattr(analysis,'analysis_duration',None),'0')} seconds</td></tr><tr><th>Total Files</th><td>{_safe(getattr(analysis,'total_files',None),'0')}</td></tr><tr><th>Total Lines</th><td>{_safe(getattr(analysis,'total_lines',None),'0')}</td></tr><tr><th>Functions</th><td>{_safe(getattr(analysis,'functions_count',None),'0')}</td></tr><tr><th>Classes</th><td>{_safe(getattr(analysis,'classes_count',None),'0')}</td></tr><tr><th>Comments</th><td>{_safe(getattr(analysis,'comments_count',None),'0')}</td></tr><tr><th>Blank Lines</th><td>{_safe(getattr(analysis,'blank_lines',None),'0')}</td></tr><tr><th>Complexity Classification</th><td>{_safe(analysis.complexity)}</td></tr></table>{project_profile_enrichment(analysis)}</section>
<section class="card" id="recommendations"><div class="section-kicker">Action plan</div><h2>Prioritized Recommendations</h2><ol class="recommendations">{recommendation_html}</ol>{recommendations_enrichment(analysis)}</section>
<section class="card page-break" id="quality-findings"><div class="section-kicker">Code quality</div><h2>Syntax and Pylint Findings</h2>{quality_enrichment()}<h3>Syntax findings</h3><pre class="output">{_preformatted(getattr(analysis,'syntax_output',None))}</pre><h3>Pylint findings</h3><pre class="output">{_preformatted(getattr(analysis,'pylint_output',None))}</pre></section>
<section class="card page-break" id="security-findings"><div class="section-kicker">Application security</div><h2>Security Findings</h2><p><strong>{security_count}</strong> security issue(s) were recorded. Current risk classification: <span class="risk {risk_class}">{risk_label}</span>.</p>{security_enrichment(analysis)}<pre class="output">{_preformatted(getattr(analysis,'bandit_output',None))}</pre></section>
<section class="card page-break" id="complexity-findings"><div class="section-kicker">Maintainability</div><h2>Complexity Findings</h2>{complexity_enrichment()}<pre class="output">{_preformatted(getattr(analysis,'radon_output',None))}</pre></section>
<section class="card page-break" id="appendix"><div class="section-kicker">Reference</div><h2>Appendix</h2><table class="table"><tr><th>Analysis ID</th><td>{analysis.id}</td></tr><tr><th>Project Record</th><td>{_safe(getattr(project,'project_id',None),str(getattr(project,'id','Not available')))}</td></tr><tr><th>Formatting Status</th><td>{_safe(getattr(analysis,'formatting_status',None))}</td></tr><tr><th>Report Filename</th><td>{filename}</td></tr><tr><th>Generated By</th><td>Sentrix by HR-Presents</td></tr></table>{appendix_enrichment(analysis)}<p class="appendix-note">This report summarizes automated static-analysis findings. Results should be reviewed by a qualified developer before production deployment. A clean result does not guarantee that software is free of defects or vulnerabilities.</p></section>
