...
<main class="report">
    ...
    <section class="card" id="project-profile">
        ...
        <table class="table">
            ...
            <tr><th>Complexity Classification</th><td>{_safe(analysis.complexity)}</td></tr>
        </table>
    </section>
    ...
    <section class="card page-break" id="quality-findings">
        <div class="section-kicker">Code quality</div>
        <h2>Syntax and Pylint Findings</h2>
    ...
    <section class="card page-break" id="security-findings">
        <div class="section-kicker">Application security</div>
        <h2>Security Findings</h2>
        <p><strong>{security_count}</strong> security issue(s) were recorded. Current risk classification: <span class="risk {risk_class}">{risk_label}</span>.</p>
        <pre class="output">{_preformatted(getattr(analysis, 'bandit_output', None))}</pre>
    </section>

    <section class="card page-break" id="complexity-findings">
    ...
    <section class="card page-break" id="appendix">
        <div class="section-kicker">Reference</div>
        <h2>Appendix</h2>
        <table class="table">
            <tr><th>Analysis ID</th><td>{analysis.id}</td></tr>
            <tr><th>Project Record</th><td>{_safe(getattr(project, 'project_id', None), str(getattr(project, 'id', 'Not available')))}</td></tr>
            <tr><th>Formatting Status</th><td>{_safe(getattr(analysis, 'formatting_status', None))}</td></tr>
            <tr><th>Report Filename</th><td>{filename}</td></tr>
            <tr><th>Generated By</th><td>Sentrix by HR-Presents</td></tr>
        </table>
        <p class="appendix-note">This report summarizes automated static-analysis findings. Results should be reviewed by a qualified developer before production deployment. A clean result does not guarantee that software is free of defects or vulnerabilities.</p>
    </section>
