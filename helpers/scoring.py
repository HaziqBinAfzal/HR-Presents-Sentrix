...
def risk_level(severity_counts):
    """Use the stable v1 finding-count thresholds everywhere."""
    count = sum(int(value or 0) for value in (severity_counts or {}).values())
    if count >= 5:
        return "High"
    if count >= 2:
        return "Medium"
...
def calculate_scorecard(*, pylint_score=None, bandit_issues=None, complexities=None,
                        syntax_error_count=0, formatting_status=None, enabled=None):
    enabled = enabled or {}
    components = {}
...
def _bandit_issues_from_output(output, fallback_count=0):
    ...
    if text:
        for block in re.split(r"\n\s*\n", text):
            ...
            if lines and lines[0].lower() in {"high", "medium", "low"}:
                issues.append({"severity": lines[0].lower()})
...
def _complexities_from_output(output, level=None):
    values = [float(match.group(1)) for match in re.finditer(
        r"Complexity\s*:\s*([0-9]+(?:\.[0-9]+)?)", str(output or ""), re.I
    )]
    if values:
        return values
    normalized = str(level or "").strip().lower()
    return {"high": [11.0], "medium": [8.0], "low": [5.0]}.get(normalized, [])
...
def scorecard_from_analysis(analysis):
    ...
    bandit_issues = _bandit_issues_from_output(
        getattr(analysis, "bandit_output", ""), getattr(analysis, "security_count", 0)
    )
    complexities = _complexities_from_output(
        getattr(analysis, "radon_output", ""), getattr(analysis, "complexity", None)
    )
